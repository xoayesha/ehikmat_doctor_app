...
const String apiUrl = 'http://10.0.2.2:8000/api/hakeems/profile/';
...
class _DoctorFormScreenState extends State<DoctorFormScreen> {
  ...
  void _cancel() => setState(() => _currentStep = _currentStep > 0 ? _currentStep - 1 : 0);
  ...
  Future<void> _submit() async {
    ...
    if (_profileImage != null) {
      ...
      profile['profile_image'] = 'data:image/jpeg;base64,${base64Encode(bytes)}';
    }
    ...
    try {
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Server error ${res.statusCode}: ${res.body}')),
        );
      }
    } catch (e) {
      if (!context.mounted) return;
  ...
  List<Step> _buildSteps() => [
        ...
        Step(
          ...
          content: Column(children: [
            ...
            _text('Full Name *', save: (v) => profile['full_name'] = v, required: true),
            _text('Phone *', save: (v) => profile['phone'] = v, kb: TextInputType.phone, required: true),
            _text('Email *', save: (v) => profile['email'] = v, kb: TextInputType.emailAddress, requiredEmail: true),
            _dropdown('Gender *', genders.map((e) => e.toLowerCase()).toList(), save: (v) => profile['gender'] = v),
        ...
        Step(
          ...
          content: Column(children: [
            _dropdown('Primary *',
                specialisations.map((s) => s.toLowerCase().replaceAll(' ', '_')).toList(),
            ...
            MultiSelectDialogField<String>(
              ...
              items: specialisations.map((s) => MultiSelectItem(s.toLowerCase().replaceAll(' ', '_'), s)).toList(),
              onConfirm: (vals) => profile['secondary_specialization'] = vals,
            ),
            _text('Experience Years', kb: TextInputType.number,
                save: (v) => profile['experience_years'] = int.tryParse(v ?? '0') ?? 0),
            _text('NCT Registration No', save: (v) => profile['nct_registration_no'] = v),
            _text('Other Accreditation', save: (v) => profile['other_accreditation'] = v),
          ]),
        ),
        ...
        Step(
          ...
          content: Column(children: [
            ...profile['clinics'].asMap().entries.map((e) => _clinicCard(e.key, e.value)),
            Align(
              alignment: Alignment.centerLeft,
              child: OutlinedButton.icon(
                  onPressed: () => setState(() => profile['clinics'].add({
                        'name': '',
                        'address': '',
                        'phone': '',
                        'assistant_cell': '',
        ...
        Step(
          ...
          content: Column(children: [
            SwitchListTile(
              ...
              onChanged: (v) => setState(() => profile['video_consultation']['is_available'] = v),
              title: const Text('Available?'),
            ),
            _text('Timings', save: (v) => profile['video_consultation']['timings'] = v),
            _text('Days', save: (v) => profile['video_consultation']['days'] = v),
            _text('Duration minutes', kb: TextInputType.number,
                save: (v) => profile['video_consultation']['duration_minutes'] = int.tryParse(v ?? '0') ?? 0),
            _text('Fees', kb: TextInputType.number,
                save: (v) => profile['video_consultation']['fees'] = int.tryParse(v ?? '0') ?? 0),
          ]),
        ),
        ...
        Step(
          ...
          content: Column(children: [
            ...profile['education_records'].asMap().entries.map((e) => _educationCard(e.key, e.value)),
            Align(
              alignment: Alignment.centerLeft,
              child: OutlinedButton.icon(
                  onPressed: () => setState(() => profile['education_records'].add({
                        'degree': '',
                        'institute': '',
                        'country': '',
        ...
        Step(
          ...
          content: Column(children: [
            ...profile['experience_records'].asMap().entries.map((e) => _experienceCard(e.key, e.value)),
            Align(
              alignment: Alignment.centerLeft,
              child: OutlinedButton.icon(
                  onPressed: () => setState(() => profile['experience_records'].add({
                        'position': '',
                        'clinic_or_hospital': '',
                        'country': '',
                        'start_year': DateTime.now().year,
        ...
        Step(
          ...
          content: Column(children: [
            MultiSelectDialogField<String>(
              ...
              items: servicesList.map((s) => MultiSelectItem(s.toLowerCase().replaceAll(' ', '_'), s)).toList(),
            ...
            MultiSelectDialogField<String>(
              ...
              items: conditionsList.map((c) => MultiSelectItem(c.toLowerCase().replaceAll(' ', '_'), c)).toList(),
        ...
        Step(
          ...
          content: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            CheckboxListTile(
              ...
              title: const Text('✔ I confirm all details provided are authentic.'),
            ),
            _text('Representative Names', save: (v) => profile['representative_names'] = v),
  ...
  Widget _text(String label,
    ...
    return TextFormField(
      ...
      validator: (v) {
        ...
        if (requiredEmail && (v == null || !v.contains('@'))) return 'Invalid email';
        return null;
      },
      onSaved: save,
    );
  ...
  Widget _dropdown(String label, List<String> values,
      {List<String>? labels, FormFieldSetter<String>? save}) {
    labels ??= values;
  ...
  Widget _clinicCard(int idx, Map<String, dynamic> clinic) => Card(
        ...
        child: Padding(
          ...
          child: Column(children: [
            ...
            _text('Phone', kb: TextInputType.phone, save: (v) => clinic['phone'] = v),
            _text('Assistant Cell', kb: TextInputType.phone, save: (v) => clinic['assistant_cell'] = v),
            _text('Practicing Since', kb: TextInputType.number, save: (v) => clinic['practicing_since'] = int.tryParse(v ?? '0') ?? 0),
            _text('Timings', save: (v) => clinic['timings'] = v),
            _text('Days', save: (v) => clinic['days'] = v),
            _text('Fees', kb: TextInputType.number, save: (v) => clinic['fees'] = int.tryParse(v ?? '0') ?? 0),
  ...
  Widget _educationCard(int idx, Map<String, dynamic> edu) => Card(
        ...
        child: Padding(
          padding: const EdgeInsets.all(8.0),
          child: Column(children: [
            _dropdown('Degree', degrees, save: (v) => edu['degree'] = v, labels: degrees),
            _text('Institute', save: (v) => edu['institute'] = v),
            _dropdown('Country', countries, save: (v) => edu['country'] = v),
            _text('Start Year', kb: TextInputType.number, save: (v) => edu['start_year'] = int.tryParse(v ?? '0') ?? 0),
            _text('End Year', kb: TextInputType.number, save: (v) => edu['end_year'] = int.tryParse(v ?? '0') ?? 0),
            Align(
              alignment: Alignment.centerRight,
              child: IconButton(
                icon: const Icon(Icons.delete, color: Colors.red),
                onPressed: () => setState(() => profile['education_records'].removeAt(idx)),
              ),
            ),
          ]),
        ),
  ...
  Widget _experienceCard(int idx, Map<String, dynamic> exp) => Card(
        ...
        child: Padding(
          ...
          child: Column(children: [
            ...
            _text('Start Year', kb: TextInputType.number, save: (v) => exp['start_year'] = int.tryParse(v ?? '0') ?? 0),
            _text('End Year', kb: TextInputType.number, save: (v) => exp['end_year'] = int.tryParse(v ?? '0') ?? 0),
            Align(
              alignment: Alignment.centerRight,
              child: IconButton(
                icon: const Icon(Icons.delete, color: Colors.red),
                onPressed: () => setState(() => profile['experience_records'].removeAt(idx)),
              ),
            ),
          ]),
        ),
